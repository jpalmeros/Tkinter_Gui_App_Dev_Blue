...
# CreateMenu2 and Submenu2
menu_butoon_2 = tk.Menubutton(
    my_menu_bar,
    text='Menu 2',
)
menu_butoon_2.pack(side=tk.LEFT)
my_menu_2 = tk.Menu(menu_butoon_2, tearoff=0)
menu_butoon_2['menu'] = my_menu_2
...
tk.Label(my_frame_1, text='Image Fun with Bimap Class:').pack()
...
# craete another frame(my+frame_2) to hold a list box, Spinbox Widget, Scale Widget, :
...
tk.Label(root, text='Below is an example of Paned window idget:').pack()
tk.Label(
    root,
    text='Notice you can adjust the size of each pane by dragging it').pack()
my_paned_window1 = tk.PanedWindow()
my_paned_window1.pack(fill=tk.BOTH, expand=2)
left_pane_text = tk.Text(my_paned_window1, height=6, width=15)
my_paned_window1.add(left_pane_text)
my_paned_window2 = tk.PanedWindow(my_paned_window1, orient=tk.VERTICAL)
my_paned_window1.add(my_paned_window2)
top_pane_text = tk.Text(my_paned_window2, height=3, width=3)
my_paned_window2.add(top_pane_text)
bottom_pane_text = tk.Text(my_paned_window2, height=3, width=3)
my_paned_window2.add(bottom_pane_text)
